...
const handler = async (m, {conn, command, args, text, usedPrefix}) => {
  ...
  try {
    ...
    const texto1 = `*◉——⌈🔊 يـوتـيـوب بـحـث 🔊⌋——◉*\n
❏ 📌 *الـعـنـوان:* ${yt_play[0].title}
❏ 📆 *تـاريـخ الـنـشـر:* ${yt_play[0].ago}
❏ ⌚ *الـمـدة:* ${secondString(yt_play[0].duration.seconds)}
❏ 👀 *الـمـشــاهـدات:* ${`${MilesNumber(yt_play[0].views)}`}
❏ 👤 *الاكـتـور:* ${yt_play[0].author.name}
❏ ⏯️ *الـقـنـاة:* ${yt_play[0].author.url}
❏ 🪬 *الـنـوع:* ${yt_play[0].type}
❏ 🔗 *الـرابـط:* ${yt_play[0].url}\n
جـاري الـتـحـمـيـل．．．_*`.trim();
    ...
    if (command == 'play4' || command == 'playdoc2') {
      try {
        ...
        await await conn.sendMessage(m.chat, {document: {url: dl_url}, fileName: `${ttl}.mp4`, mimetype: 'video/mp4', caption: `▢ الـعـنـوان: ${ttl}\n▢ الـمـسـاحـه: ${size}`, thumbnail: await fetch(yt.thumbnail)}, {quoted: m});
      } catch {
        try {
          ...
          try {
            ...
            await conn.sendMessage(m.chat, {document: {url: n2}, fileName: `${n}.mp4`, mimetype: 'video/mp4', caption: `▢ الـعـنـوان: ${n}\n▢ الـمـسـاحـه: ${n3}`, thumbnail: await fetch(n4)}, {quoted: m});
          } catch {
            await conn.reply(m.chat, '*❐┃عـذرا. هـنـاك خـطـئ┃❌ ❯*', m);
          }
        }
      }
    }
  } catch {
    throw '*❐┃عـذرا. هـنـاك خـطـئ┃❌ ❯*';
  }
};
handler.help = ['play3', 'play4'].map((v) => v + ' < busqueda >');
handler.tags = ['downloader'];
handler.command = /^(بحث_يوتيوب|play3|play4)$/i;
